...
import { getDay, getDaysInMonth, isBefore, startOfToday, isWithinInterval, startOfDay, endOfDay } from 'date-fns';
...
export const monthsForLocale = (
  ...
  return [...new Array(12).keys()].map((m) =>
    formatter.dateTime(new Date(Date.UTC(2021, m, 2)), { month: monthFormat })
  );
};
...
export const daysForLocale = (
  ...
  for (let i = 0; i < 7; i++) {
    weekdays.push(
      formatter.dateTime(baseDate, { weekday: 'short' })
    );
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      result[day] = features.filter((feature) => {
        return isWithinInterval(currentDate, {
          start: startOfDay(feature.startAt),
          end: endOfDay(feature.endAt)
        });
      });
    }
    return result;
    ...
    days.push(
      <div
        className={cn(
          ...
          !isPast && "cursor-pointer hover:bg-accent/50",
...
export const CalendarMonthPicker = ({
  ...
  const monthData = useMemo(() => {
    return monthsForLocale(formatter).map((month, index) => ({
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
  ...
  const daysData = useMemo(() => {
    return daysForLocale(formatter, startDay);
  }, [formatter, startDay]);
...
export const CalendarProvider = ({
  ...
  <CalendarContext.Provider value={{ startDay }}>
